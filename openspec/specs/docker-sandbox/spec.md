...
The docker-sandbox capability provides optional Docker-based filesystem isolation for the agent, restricting LLM tool calls to only access files within the current working directory. This security boundary protects sensitive files outside the project scope while maintaining full agent functionality.
