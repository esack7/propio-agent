...
function getOpenRouterApiKey(): string | undefined {
  ...
  try {
    ...
    const openRouter = config.providers?.find((p: any) => p.type === 'openrouter');
...
describe('OpenRouter integration (real API)', () => {
  ...
  describe('9.2 Non-streaming chat with real API', () => {
    itIntegration('should complete non-streaming chat with openai/gpt-3.5-turbo', async () => {
      const provider = createProvider(openRouterProviderConfig);
      ...
      expect(['end_turn', 'tool_use', 'max_tokens', 'stop_sequence']).toContain(response.stopReason);
    }, 15000);
  });

  describe('9.3 Streaming chat with real API', () => {
    itIntegration('should stream chat and yield incremental chunks', async () => {
      const provider = createProvider(openRouterProviderConfig);
      const chunks: string[] = [];
  ...
  describe('9.4 Tool calling with real API', () => {
    itIntegration('should handle tool calling with openai/gpt-3.5-turbo', async () => {
      const provider = createProvider(openRouterProviderConfig);
      const response = await provider.chat({
        model: 'openai/gpt-3.5-turbo',
        messages: [{ role: 'user', content: 'What is 2+2? Use no tools, just answer in one number.' }],
        tools: [
          {
            type: 'function',
            function: {
      ...
      expect(response.message.role).toBe('assistant');
      expect(response.message.content !== undefined || (response.message.toolCalls && response.message.toolCalls.length > 0)).toBe(true);
    }, 15000);
  });

  describe('9.5 Invalid API key', () => {
    itIntegration('should throw ProviderAuthenticationError for invalid API key', async () => {
  ...
  describe('9.6 Runtime provider switching', () => {
    itIntegration('should switch between OpenRouter and another OpenRouter config and preserve context', async () => {
      ...
      const response = await agent.chat('What number did I ask you to remember? Reply with just the number.');
