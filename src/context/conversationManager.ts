...
/**
 * ConversationManager owns reusable conversational state.
 *
...
export class ConversationManager {
  ...
  constructor({
    toolResultSummaryMaxChars = DEFAULT_SUMMARY_MAX_CHARS,
    rehydrationMaxChars = DEFAULT_REHYDRATION_MAX_CHARS,
    ...
    artifactLookup?: (id: string) => ArtifactRecord | undefined;
  } = {}) {
    this.tokenEstimator = tokenEstimator;
    this.artifactLookup = artifactLookup;
    this.promptBuilder = new PromptBuilder(tokenEstimator);
    this.summaryMaxChars = toolResultSummaryMaxChars;
    this.rehydrationMaxChars = rehydrationMaxChars;
    this.pinnedMemoryMaxContentLength = pinnedMemoryMaxContentLength;
  }
  ...
  protected discardIncompleteTurnWhen(
  ...
  commitAssistantResponse(
    ...
    turn.entries.push({
      ...
      estimatedTokens: this.tokenEstimator.estimateMessages([message]),
      message,
    });
  ...
  recordToolResults(results: ArtifactToolResult[]): void {
    ...
    for (const result of results) {
      ...
      const artifact: MutableArtifact = {
        ...
        estimatedTokens: this.tokenEstimator.estimateCharacters(size),
  ...
  private normalizePromptPlanOptions(
    ...
    return {
      ...
      supplementalContext: options?.supplementalContext,
    };
  }
  ...
  private createPromptBuildRequest(
    ...
    return {
      ...
      artifactLookup: (id: string) =>
        this.artifactLookup?.(id) ?? this.artifacts.get(id),
  ...
  private completeTurn(turn: MutableTurnRecord): void {
    ...
    turn.estimatedTokens = this.tokenEstimator.estimateMessages([
      turn.userMessage,
      ...turn.entries.map((entry) => entry.message),
    ]);
  }
}
