...
Invoked skills receive stable invocation IDs that survive session save and recovery. Configuration revisions record skill activation and clearing; selected tool scopes and prompt-plan instruction revisions identify each active invocation, while provider requests reference those scope and prompt revisions. Repeated invocations of the same skill remain distinguishable. Legacy sessions without invocation IDs receive deterministic trace-only IDs. Standard trace metadata includes skill names, sources, timestamps, and content revision hashes, but not skill bodies.

Tool policy records include a keyed fingerprint of the reviewed arguments, matched by the dispatch record; full capture separately retains the arguments as private material. The fingerprint uses an ephemeral runtime key and the tool-call ID, so standard traces can compare records for one call without exposing a reproducible hash of candidate arguments. The key is not persisted, and fingerprints are not comparable across runtime instances or different tool calls. Cancellation before authorization or during an approval callback is recorded as `cancelled`, distinct from denial and callback failure. A denial already recorded for a call is not followed by a second cancellation decision if the turn is cancelled before dispatch. No cancelled or denied tool is dispatched.

The provisional `@propio-ai/agent/trace` entry point exposes explicit-path `JsonlTraceJournal`, `RunTraceRecorder`, tolerant journal reading, offline inspection, portable export, and bundle hash verification. Importing it performs no discovery or provider/tool execution. `inspectTraceJournal` retains valid records when the final JSONL line is truncated and marks unresolved operations unknown. `exportTraceJournal` writes a versioned manifest, its `manifest.sha256` checksum, and relative `events.jsonl`; `verifyTraceExport` rejects unsafe paths, file hash/size mismatches, changed manifests, and v3 run claims that disagree with the exported events. For offline bundle inspection, call `inspectTraceExport(bundleDirectory)`: it verifies the bundle and applies the source-journal warnings retained in the manifest. Calling `inspectTraceJournal` on the bundle's `events.jsonl` alone sees only its valid exported events and cannot know that a malformed source line was dropped. Export re-applies standard redaction and omits malformed journal lines while recording their warnings, so an interrupted bundle remains inspectable after moving it away from the original machine. Consequently, `events.jsonl` is a redacted re-serialization, not a byte-for-byte copy of the source journal. The checksum and consistency checks detect accidental or one-sided edits, but a bundle is not authenticated against someone who can rewrite both its contents and checksums; that requires a signature or an externally trusted digest.

Inspection also deduplicates cumulative provider reports and aggregates usage across the run, request purpose, and retry attempts. Completeness is reported as `complete`, `partial`, or `unavailable`, including for each token metric; a missing metric never becomes zero, and unfinished or truncated captures cannot claim complete totals. Provider-reported charges remain separate from local estimates. Applications may pass a `ProviderPricingResolver` to `inspectTraceJournal` or `exportTraceJournal`; every persisted estimate includes the resolver source, version, effective date, requested and actual model context, endpoint class, usage, and redacted calculation inputs. No default resolver is installed, and locally hosted inference is not assumed to be free. Export manifest version 3 adds operation summaries, configuration/prompt and package revision metadata, a per-event material inventory, and completeness warnings; verification still accepts versions 1 and 2 with their weaker legacy checks. The journal material entry is marked `partial` when malformed source lines were dropped. Standard exports intentionally omit raw tool results, prompt artifacts, and provider payloads. The optional `artifact_pruned` reason code allows embedding applications to report pruning, but the built-in agent does not currently emit artifact-pruning records. Workspace baseline and diff are currently reported as missing because they are not recorded; a bundle must not be interpreted as a full workspace snapshot.

The workspace limitation above applies to standard exports.

### Private full trace capture

Set `PROPIO_TRACE_CAPTURE_LEVEL=full` when starting the CLI to capture the application-visible request, per-attempt provider-adapter payload, streamed provider events, tool arguments and complete tool results, and workspace files. The sandbox wrapper forwards this setting. Each run stores content-addressed material beside its journal in a private `*.materials/` directory. The per-attempt payload is the HTTP JSON body or SDK input object before SDK serialization; it reflects retry-time changes, but it is not a byte-for-byte wire capture and does not include authentication headers. A workspace baseline is captured before the turn, with cumulative changes captured after each committed tool result and at run end. Unchanged files are reused by hash. The workspace scope includes Git-tracked and untracked files that are not ignored; without Git it walks the workspace except `.git`, `node_modules`, `dist`, and `coverage`. Credential-shaped paths such as `.env`, `.npmrc`, and private key files are omitted and reported. A missing or omitted file makes the full export partial.

To create an offline bundle that includes the captured material, call `exportTraceJournal(journalPath, destinationDirectory, { captureLevel: "full" })` from `@propio-ai/agent/trace`. The version 4 manifest inventories the included files and their SHA-256 hashes and names the source run's material directory; references into sibling runs are not copied and make the export incomplete. `verifyTraceExport` checks the bundle and `inspectTraceExport` works after the source directory is moved. The existing call without `captureLevel` continues to produce a standard version 3 bundle. `loadRecordedProviderResponse(bundleDirectory, requestId)` returns the completed normalized stream events from a verified full bundle; `playRecordedProviderResponse(bundleDirectory, requestId)` yields them as an async iterable. Both are read-only and make no provider or tool calls. An incomplete or missing response is rejected. Full bundles can contain source files, prompts, and tool output, including secrets embedded in ordinary text, so treat them as private data.

Run `npm run benchmark:trace` to build and check the documented development regression limits: 1,000 streamed text chunks captured as one response in at most 1 second and 1 MB, 100 tool completions with 4 KiB results in at most 5 seconds and 2 MB, and a 100-file workspace with 20 changes in at most 5 seconds and 5 MB. These are generous ceilings for an opt-in diagnostic mode; the script prints the measured time and disk growth for each fixture. On a local macOS run, the 100-tool fixture took 0.71 seconds and 31 KB in standard mode versus 3.12 seconds and 489 KB in full mode, roughly 24 ms of additional durable capture work per completed tool. The 100-file workspace fixture took 1.09 seconds and 51 KB because identical file content is stored once. Results vary with storage hardware and filesystem sync behavior.
